...
void test_04()
{
    ...
    //-----------------------------------------------------
    std::string ThreadName("test_log");

    //利用宏
    //使用流式方式将日志级别level的日志写入到logger
    SYLAR_LOG_DEBUG(logger, ThreadName) << "@debug@";
    SYLAR_LOG_INFO(logger, ThreadName) << "@info@";
    SYLAR_LOG_WARN(logger, ThreadName) << "@warn@";
    SYLAR_LOG_ERROR(logger, ThreadName) << "@error@";
    SYLAR_LOG_FATAL(logger, ThreadName) << "@fatal@";

    //利用宏
    //使用格式化方式将日志级别level的日志写入到logger
    SYLAR_LOG_FMT_DEBUG(logger, ThreadName, "test macro fmt error %s", "@debug@");
    SYLAR_LOG_FMT_INFO(logger, ThreadName, "test macro fmt error %s", "@info@");
    SYLAR_LOG_FMT_WARN(logger, ThreadName, "test macro fmt error %s", "@warn@");
    SYLAR_LOG_FMT_ERROR(logger, ThreadName, "test macro fmt error %s", "@error@");
    SYLAR_LOG_FMT_FATAL(logger, ThreadName, "test macro fmt error %s", "@fatal@");

    //-----------------------------------------------------
    auto LL = sylar::LoggerMgr::GetInstance()->getLogger("XX");
    SYLAR_LOG_INFO(LL, ThreadName) << "@XXXXXXXXXXXXXXXXXXXXXX@";
