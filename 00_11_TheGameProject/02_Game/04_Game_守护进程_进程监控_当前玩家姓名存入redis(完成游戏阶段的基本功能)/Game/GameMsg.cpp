...
MultiMsg::~MultiMsg()
{
    std::list<GameMsg *>::iterator itr_singleGameMsg = m_Msgs.begin();
    while (itr_singleGameMsg != m_Msgs.end())
    {
        if (nullptr != (*itr_singleGameMsg))
        {
            GameMsg *p_tmp = *itr_singleGameMsg;
            itr_singleGameMsg = m_Msgs.erase(itr_singleGameMsg);
            delete (p_tmp);
            p_tmp = nullptr;
        }
        else
    ...
    // for(auto single_GameMsg : m_Msgs)
    // {
    //     delete single_GameMsg;
    //     single_GameMsg = nullptr;
    // }
}
