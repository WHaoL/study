...
bool GameRole::Init()
{
    ...
    //记录当前姓名到文件(追加)
    //std::ofstream name_record_file("/tmp/name_record_file",std::ios_base::app);
    //name_record_file << szName << std::endl;
    /*记录当前姓名到redis的game_name_list(list类型的key)*/
    //1 连接redis
    /* connect redis */
    ...
    redisReply *reply = (redisReply *)redisCommand(C, "LPUSH game_name_list %s", szName.c_str());
    freeReplyObject(reply);
    /* Let's check what we have inside the list */
    reply = (redisReply *)redisCommand(C, "LRANGE game_name_list 0 -1");
...
void GameRole::Fini()
{
    ...
    // std::ifstream input_stream("/tmp/name_record_file");
    // std::string tmp;
    // while (getline(input_stream, tmp))
    // {
    //     cur_name_list.push_back(tmp);
    // }
    // std::ofstream output_name("/tmp/name_record_file");
    // for (std::string name : cur_name_list)
    // {
    //     if (name != szName) //非自己，重新写入
    //     {
    //         output_name << name << std::endl;
    //     }
    // }

    /*从redis的game_name_list(list类型的key)删除自己的名字*/
    //1 连接redis
    /* connect redis */
    redisContext *C = redisConnect("127.0.0.1", 6379);
    if (C == NULL || C->err)
    ...
    //2 发送lpush命令
    /* Create a list of name */
    redisReply *reply = (redisReply *)redisCommand(C, "lrem game_name_list 1 %s", szName.c_str());
    freeReplyObject(reply);
    /* Let's check what we have inside the list */
    reply = (redisReply *)redisCommand(C, "LRANGE game_name_list 0 -1");
    if (reply->type == REDIS_REPLY_ARRAY)
